...
def create_track_chart(
    ...
    """
    Create and display/save a track chart showing equity curve and drawdown over time.
    ...
    """
    if not MATPLOTLIB_AVAILABLE:
        print("Skipping track chart generation (matplotlib not available)")
        print("Install matplotlib to generate charts: pip install matplotlib")
        return
    ...
    ax1.tick_params(axis="y", labelcolor=color1)
    ax1.grid(True, alpha=0.3)
    ax1.set_title(f"Portfolio Track Analysis - {strategy_name}", fontsize=14, fontweight="bold")
    ...
    if output_dir:
        ...
        safe_name = safe_name.replace(" ", "_")[:50]
        filename = f"{safe_name}_track_chart.png"
        filepath = os.path.join(output_dir, filename)
        plt.savefig(filepath, dpi=150, bbox_inches="tight")
        print(f"Track chart saved to: {filepath}")
